...
(function () {
      ...
      try {
        globalThis.dispatchEvent(
          new CustomEvent('bb_metrika_feature_used', {
            detail: { feature: 'theme', action: isLight ? 'dark' : 'light' },
          }),
        );
      } catch (_) {
        /* noop */
  ...
  async function copy(elementId, successMessage) {
    ...
    const getLoc = (key, fallback) =>
      i18n && i18n.t(key) !== key ? i18n.t(key) : fallback;
    ...
    try {
      ...
      if (globalThis.showSuccessNotification) {
        ...
        const msg =
          successMessage ||
          getLoc('controller.linkCopied', 'Ссылка скопирована');
