...
class BreathingEngine {
  constructor(options = {}) {
    ...
    this.totalCycleSec = this.inhaleSec + this.holdAfterInhale + this.exhaleSec + this.holdAfterExhale
  ...
  start(minutes = 5) {
    ...
    this.targetCycles = minutes > 0 ? Math.round((minutes * 60) / this.totalCycleSec) : 0
    this._initAudio()
  ...
  _stopAudio() {
    if (this._leftOsc) {
      try { this._leftOsc.stop() } catch (e) { /* ignore */ }
      this._leftOsc = null
    }
    if (this._rightOsc) {
      try { this._rightOsc.stop() } catch (e) { /* ignore */ }
      this._rightOsc = null
    }
    this._leftGain = null
    this._rightGain = null
  ...
  getStats() {
    const es = this.running ? (performance.now() - this.startTimestamp) / 1000 : 0
    return {
