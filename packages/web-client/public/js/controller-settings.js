  ...
  updateHeaderSessionName() {
    try {
      ...
      if (timestampEl) {
        ...
        if (i18n?.isReady && i18n.t('controller.sessionCreated') !== 'controller.sessionCreated') {
          timestampEl.textContent = `${i18n.t('controller.sessionCreated')}`
        } else {
          timestampEl.textContent = `Created: ${dateStr}`
        }
      }
