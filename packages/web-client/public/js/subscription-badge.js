...
(function () {
  ...
  const STRINGS = {
    activeText: function () {
      return t('subscriptionBadge.active', 'PRO');
    },
    activeTitle: function () {
      return t('subscriptionBadge.activeTitle', 'Active subscription');
    },
    inactiveText: function () {
      return t('subscriptionBadge.inactive', 'Support us');
    },
    inactiveTitle: function () {
      return t(
        'subscriptionBadge.inactiveTitle',
        'Support the project — 75⭐ / 30 days',
      );
    },
    dialogTitle: function () {
      return t('subscriptionBadge.dialogTitle', 'What you get with Supporter');
    },
    dialogClose: function () {
      return t('subscriptionBadge.dialogClose', 'Close');
    },
    planName: function () {
      return t('subscription.planName', 'Supporter');
    },
    benefit1: function () {
      return t('subscription.benefit1', 'Custom permanent session IDs');
    },
    benefit2: function () {
      return t(
        'subscription.benefit2',
        'Links never expire — reuse with same client',
      );
    },
    benefit3: function () {
      return t('subscription.benefit3', 'Priority support');
    },
    benefitSupport: function () {
      return t(
        'subscription.benefitSupport',
        'Helps keep BilateralBound running',
      );
    },
    price: function () {
      return t('subscription.price', '75 ⭐ / 30 days');
    },
    cta: function () {
      return t('subscription.cta', 'Support with 75 ⭐');
    },
    ctaNote: function () {
      return t(
        'subscription.ctaNote',
        'One-time via Telegram — no auto-charge',
      );
    },
    alreadyActive: function () {
      return t('subscription.alreadyActive', 'Supporter Active');
    },
    manageInBot: function () {
      return t('subscription.manageInBot', 'Manage in Telegram Bot');
    },
  };
  ...
  function refresh() {
    ...
    badge.setAttribute(
      'aria-label',
      isActive ? STRINGS.activeTitle() : STRINGS.inactiveTitle(),
    );
  ...
  function openDialog() {
    ...
    if (isActive) {
      ...
      verifyDivider.textContent = t(
        'subscriptionBadge.orVerify',
        'or verify existing subscription',
      );
      ...
      verifyInput.placeholder = t(
        'subscriptionBadge.telegramIdPlaceholder',
        'Telegram User ID',
      );
      ...
      verifyBtn.addEventListener('click', async function () {
        ...
        if (!raw) {
          verifyMsg.textContent = t(
            'subscriptionBadge.errorEmpty',
            'Please enter your Telegram User ID',
          );
          verifyMsg.className =
            'sub-dialog__verify-msg sub-dialog__verify-msg--error';
          return;
        }
        const tgId = parseInt(raw, 10);
        if (!tgId || tgId <= 0) {
          verifyMsg.textContent = t(
            'subscriptionBadge.errorInvalid',
        ...
        try {
          ...
          if (resp.ok) {
            verifyMsg.textContent = t(
              'subscriptionBadge.verifySuccess',
              'Subscription verified! Reloading…',
            );
            verifyMsg.className =
              'sub-dialog__verify-msg sub-dialog__verify-msg--success';
            setTimeout(function () {
            ...
            verifyMsg.textContent =
              (data.i18nKey
                ? t(data.i18nKey, data.message || data.error)
                : null) ||
              t('subscriptionBadge.verifyFailed', 'Verification failed');
            verifyMsg.className =
              'sub-dialog__verify-msg sub-dialog__verify-msg--error';
          }
        } catch (_) {
          verifyMsg.textContent = t(
            'subscriptionBadge.errorNetwork',
    ...
    requestAnimationFrame(function () {
      wrap.classList.add('sub-dialog--show');
    });
  ...
  async function checkSubscription() {
    ...
    let proofId = null;
    try {
      proofId = localStorage.getItem('subscriptionProofId');
    } catch {
      /* ignore */
    }
    if (proofId) {
      try {
        ...
        const respProof = await fetchProof(
          '/api/subscription/' + encodeURIComponent(proofId) + '/check',
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        const proofData = await respProof.json().catch(function () {
          return {};
        });
        if (proofData.active) {
          badge.classList.remove('sub-badge--loading');
    ...
    try {
      const fetchFn = globalThis.csrfFetch || fetch;
      const resp = await fetchFn(
        '/api/subscription/' + encodeURIComponent(SESSION_ID) + '/check',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'same-origin',
      ...
      /* continue to link-access fallback */
    }

    // Fallback: check if browser was unlocked via bb_lk cookie (link-access)
    try {
      const resp2 = await fetch(
        '/api/link-access/' + encodeURIComponent(SESSION_ID) + '/check',
        {
          method: 'GET',
          credentials: 'same-origin',
        },
      );
      const la = await resp2.json().catch(function () {
        return {};
      });
