(function() {
  ...
  async function createSession() {
    ...
    try {
      ...
      var connectResponse = await fetch('/api/session/' + sessionId + '/controller/connect', {
        method: 'POST',
      ...
      if (!connectResponse.ok) {
        var connectError = await connectResponse.text();
  ...
  function updateValidationMessage(message, isError) {
    var msgElement = document.getElementById('linkValidationMessage');
    if (msgElement) {
  ...
  async function generatePermanentLinks() {
    var input = document.getElementById('customClientId');
    var btn = document.getElementById('generateLinksBtn');
    var container = document.getElementById('generatedLinksContainer');
    var viewerUrlInput = document.getElementById('generatedViewerUrl');
    var controllerUrlInput = document.getElementById('generatedControllerUrl');
    ...
    try {
      ...
      var response = await fetch('/api/session/' + clientId + '/reserve', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      });
  ...
  async function loadSession() {
    var input = document.getElementById('existingSessionId');
    var btn = document.getElementById('loadSessionBtn');
    var messageEl = document.getElementById('loadSessionMessage');
    var container = document.getElementById('restoredLinksContainer');
    var viewerUrlInput = document.getElementById('restoredViewerUrl');
    var controllerUrlInput = document.getElementById('restoredControllerUrl');
    ...
    var sessionId = input.value.trim();
    ...
    try {
      ...
      var response = await fetch('/api/session/' + sessionId + '/state');
      if (!response.ok) {
  ...
  function initThemeToggle() {
    ...
    if (themeToggleBtn && window.themeManager) {
      var newBtn = themeToggleBtn.cloneNode(true);
  ...
  function init() {
    ...
    document.querySelectorAll('[data-action]').forEach(function(btn) {
      btn.addEventListener('click', function() {
        ...
        if (action === 'create-session') createSession();
          ...
          var input = this.closest('.link-group__input-wrapper, .input-group')?.querySelector('input');
          if (input) {
            input.select();
    ...
    var clientIdInput = document.getElementById('customClientId');
    if (clientIdInput) {
