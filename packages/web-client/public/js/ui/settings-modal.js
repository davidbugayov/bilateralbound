...
(function () {
  ...
  function detectCurrentLang() {
    ...
    return localStorage.getItem('emdr-language') || navigator.language.split('-')[0] || 'en'
  }

  function updateLangActive() {
    const cur = detectCurrentLang()
    langBtns.forEach(function (b) { b.classList.toggle('active', b.dataset.lang === cur) })
  }

  langBtns.forEach(function (btn) {
    btn.addEventListener('click', function () {
      const lang = btn.dataset.lang
      try { localStorage.setItem('emdr-language', lang) } catch (_) { /* ignore */ }
      try {
  ...
  function setupMirrors() {
    ...
    document.querySelectorAll('[data-mirror-display]').forEach(function (span) {
      ...
      if (!src) return
      new MutationObserver(function () { span.textContent = src.textContent })
  ...
  if (volSlider) {
    ...
    volSlider.addEventListener('input', function () { updateSliderFill(volSlider) })
  }
})()
