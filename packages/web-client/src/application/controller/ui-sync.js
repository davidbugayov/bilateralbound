...
function syncSize(ballState) {
  if (ballState.radius !== undefined && _components.size?.setSize) {
    ...
    const closest = sizes.reduce(
      (p, c) =>
        Math.abs(c - ballState.radius) < Math.abs(p - ballState.radius)
          ? c
          : p,
      sizes[0]
    )
...
module.exports = { init, syncAll, syncSpeed, syncSize, syncColors, syncPause, syncDirection, syncSound, getDirectionMode, setIgnorePausedUntil, setIgnoreDirectionUntil }
