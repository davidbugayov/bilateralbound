  ...
  app.post('/api/session/:sessionId/reserve', (req, res) => {
    ...
    try {
      if (
        ...
        if (
          proofCustomId &&
          subscriptionService.isCustomIdAllowed(proofCustomId)
        ) {
          // Auto-link the new sessionId to the same subscription owner
          const proofOwner =
            subscriptionService.getCustomIdOwner(proofCustomId)
          if (proofOwner) {
        ...
        if (!subscriptionService.isCustomIdAllowed(sessionId)) {
          return res.status(402).json({
            ...
            message:
              'Permanent session links require an active subscription. Support the project via @emdrbilateral_bot'
          })
        }
      }
