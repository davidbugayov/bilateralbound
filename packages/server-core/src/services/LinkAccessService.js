...
class LinkAccessService {
  ...
  _ensureDataDir() {
    try {
      ...
      this.logger.warn({ err }, 'Could not create data directory for link-access')
    }
  }
  ...
  _loadFromDisk() {
    try {
        ...
        for (const [browserId, perBrowser] of Object.entries(data.entries)) {
          ...
          for (const [sessionId, record] of Object.entries(perBrowser)) {
            ...
            clean.set(sessionId, {
              ...
              unlockedUntil: (record.unlockedUntil && record.unlockedUntil > now) ? record.unlockedUntil : null
            })
            hasValid = true
          }
          if (hasValid) {
      ...
      this.logger.warn({ err }, 'Failed to load link-access data, starting fresh')
    }
  }
