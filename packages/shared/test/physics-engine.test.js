...
test('creates engine with custom world size', () => {
  const engine = new PhysicsEngine({ worldWidth: 1000, worldHeight: 800, isViewer: true })
...
test('higher speed produces more displacement at short interval', () => {
  const engine1 = new PhysicsEngine({ isViewer: true, worldWidth: 800, worldHeight: 600 })
  const engine2 = new PhysicsEngine({ isViewer: true, worldWidth: 800, worldHeight: 600 })
  ...
  assert.ok(d2 > d1, `Higher speed should produce more displacement: d1=${d1}, d2=${d2}`)
})

// --- Bounce ---
test('ball bounces off left wall', () => {
  const engine = new PhysicsEngine({ worldWidth: 800, worldHeight: 600, isViewer: true })
  engine.applyCommand({ paused: false, speed: 5000, dirX: -1, dirY: 0 })
  for (let i = 0; i < 30; i++) {
    engine.update(16)
  }
  assert.ok(engine.ball.x >= engine.ball.radius, `Ball should not pass left wall: x=${engine.ball.x}`)
})
...
test('ball bounces off right wall', () => {
  ...
  assert.ok(engine.ball.x <= maxX, `Ball should not pass right wall: x=${engine.ball.x}, max=${maxX}`)
})
...
test('ball stays within vertical bounds after bounce', () => {
  ...
  assert.ok(engine.ball.y <= yMax, `Ball should not pass bottom: y=${engine.ball.y}, max=${yMax}`)
})
...
test('ball never leaves screen at extreme speed', () => {
  ...
  assert.ok(engine.ball.x >= engine.ball.radius, `Ball X below min: ${engine.ball.x}`)
  assert.ok(engine.ball.x <= engine.options.worldWidth - engine.ball.radius, `Ball X above max: ${engine.ball.x}`)
...
test('seekingCenter triggers on pause in viewer mode', () => {
  ...
  assert.strictEqual(engine.state.paused, true)
  assert.strictEqual(engine.state.seekingCenter, true, 'seekingCenter should be true')
})
...
test('seekingCenter starts on pause with returnToCenter', () => {
  ...
  assert.ok(distBefore > 10, `Ball should be far from center: dist=${distBefore}`)
  engine.applyCommand({ paused: true, returnToCenter: true })
  assert.strictEqual(engine.state.seekingCenter, true, 'seekingCenter should be true on pause')
})
...
test('radius clamped to max', () => {
  ...
  assert.ok(engine.ball.radius <= 1000, `Radius should be clamped: ${engine.ball.radius}`)
})
...
test('invalid color is rejected', () => {
  ...
  assert.strictEqual(engine.colors.ball, before, 'Invalid color should be ignored')
})
