...
async function main() {
  ...
  console.log('\n📋 Test 1: No subscription initially')
  assert(
    'isActive returns false for unknown user',
    subsvc.isActive(TELEGRAM_USER_ID) === false
  )
  assert(
    'isCustomIdAllowed returns false for unlinked customId',
  ...
  assert('activate() returns success: true', activateResult.success === true)
  assert(
    'activate() returns expiresAt (number)',
    typeof activateResult.expiresAt === 'number'
  )
  assert(
    'isActive() returns true after activation',
  ...
  assert('linkCustomId() returns success: true', linkResult.success === true)
  assert(
    'isCustomIdAllowed(CUSTOM_ID_1) returns true',
  ...
  assert(
    'linkCustomId(CUSTOM_ID_2) returns success: true',
    linkResult2.success === true
  )
  assert(
    'isCustomIdAllowed(CUSTOM_ID_2) returns true',
    subsvc.isCustomIdAllowed(CUSTOM_ID_2) === true
  )
  assert(
    'isCustomIdAllowed(CUSTOM_ID_1) still true',
    subsvc.isCustomIdAllowed(CUSTOM_ID_1) === true
  )
  ...
  assert(
    'isActive(unknownUser) returns false',
    subsvc.isActive(TELEGRAM_USER_ID_2) === false
  )
  const linkResult3 = subsvc.linkCustomId(CUSTOM_ID_3, TELEGRAM_USER_ID_2)
  assert(
    'linkCustomId(CUSTOM_ID_3) success for unsubscribed user',
    linkResult3.success === true
  )
  assert(
    'isCustomIdAllowed(CUSTOM_ID_3) returns false (no subscription)',
  ...
  assert(
    'getStatus().customIds includes CUSTOM_ID_1',
    status.customIds.includes(CUSTOM_ID_1)
  )
  assert(
    'getStatus().customIds includes CUSTOM_ID_2',
  ...
  assert(
    'Reusing same token returns success (idempotent)',
  ...
  assert(
    'isActive() returns false after expiry',
    subsvc.isActive(TELEGRAM_USER_ID) === false
  )
  assert(
    'isCustomIdAllowed(CUSTOM_ID_1) returns false after expiry',
    subsvc.isCustomIdAllowed(CUSTOM_ID_1) === false
  )
  assert(
    'isCustomIdAllowed(CUSTOM_ID_2) returns false after expiry',
  ...
  assert(
    'reactivate() returns success: true',
  ...
  assert(
    'isCustomIdAllowed(CUSTOM_ID_1) true after reactivation',
    subsvc.isCustomIdAllowed(CUSTOM_ID_1) === true
  )
  ...
  assert(
    'getStatusForCustomId("nonexistent").active === false',
  ...
  assert(
    'isActive(TELEGRAM_USER_ID) true after reactivation',
    subsvc.isActive(TELEGRAM_USER_ID) === true
  )

  // Link a brand new customId (simulating a new client from another computer)
  const CUSTOM_ID_CROSS = 'cross_computer_test'
  const linkCrossResult = subsvc.linkCustomId(
    CUSTOM_ID_CROSS,
    TELEGRAM_USER_ID
  )
  assert(
    'linkCustomId(CUSTOM_ID_CROSS) success',
    linkCrossResult.success === true
  )
  assert(
    'isCustomIdAllowed(CUSTOM_ID_CROSS) true (cross-computer)',
