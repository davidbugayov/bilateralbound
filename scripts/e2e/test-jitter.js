...
async function runJitterTest() {
  ...
  const dtVariance =
    deltas.reduce((s, d) => s + (d.dt - avgDt) ** 2, 0) / deltas.length
  ...
  if (passed) {
    ...
    console.log(
      `   (avg jitter < ${JITTER_THRESHOLD}px, dt stddev < ${DT_STDDEV_THRESHOLD}ms)`
    )
  } else {
    console.log('⚠️ WARN: Jitter exceeds recommended limits')
    if (avgJitterX >= JITTER_THRESHOLD || avgJitterY >= JITTER_THRESHOLD) {
      console.log(
        `   Average jitter (${avgJitterX.toFixed(2)}, ${avgJitterY.toFixed(2)}) exceeds ${JITTER_THRESHOLD}px`
      )
    }
    if (dtStdDev >= DT_STDDEV_THRESHOLD) {
      console.log(
        `   dt stddev (${dtStdDev.toFixed(2)}ms) exceeds ${DT_STDDEV_THRESHOLD}ms`
      )
    }
  }
