...
async function test(name, fn) {
  try {
    await Promise.race([
      fn(),
      new Promise((_, r) =>
        setTimeout(() => r(new Error('Timeout: ' + name)), 15000),
      ),
    ]);
    console.log(`\u2705 ${name}`);
    passed++;
...
async function main() {
  ...
  await test('Normal network: viewer ball moves smoothly', async () => {
    ...
    if (metrics.teleportCount > 2) {
      throw new Error(
        `Too many teleports: ${metrics.teleportCount} (threshold: 2)`,
      );
    }
  ...
  await test('Slow network (100ms): ball continues smooth (local physics)', async () => {
    ...
    if (positions.length < 3) {
      throw new Error(
        `Too few samples under slow network: ${positions.length}`,
      );
    }
    ...
    if (metrics.teleportCount > 5) {
      throw new Error(
        `Too many teleports under slow network: ${metrics.teleportCount}`,
      );
    }
  });
  ...
  await test('Viewer reconnect: ball state restores correctly', async () => {
    await viewPage.evaluate(() => {
      if (
        globalThis.wsClient &&
        typeof globalThis.wsClient.disconnect === 'function'
      ) {
    ...
    if (positions.length < 3) {
      throw new Error(
        `Viewer did not track ball after reconnect: ${positions.length} samples`,
      );
    }
