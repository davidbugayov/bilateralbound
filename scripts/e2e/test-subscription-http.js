...
const { registerSubscriptionRoutes } = require('../../packages/server-core/src/controllers/subscriptionController')
const { registerSessionRoutes } = require('../../packages/server-core/src/controllers/sessionController')
...
class FakeRepo {
  ...
  findById(id) { return this.sessions.get(id) || null }
  ...
  create(opts) { const s = { id: 'auto_' + Date.now(), ...opts }; this.sessions.set(s.id, s); return s }
  cleanupExpired() { return [] }
}

function makeSessionService(subscriptionService) {
  const repo = new FakeRepo()
  const broadcast = { broadcastState: () => {}, broadcastViewerStatus: () => {},
    broadcastViewerConnection: () => {}, broadcastControllerConnection: () => {},
    broadcastLanguageUpdate: () => {}, broadcastInitialState: () => {},
    clearDeltaCache: () => {} }
  const physics = { initializeEngine: () => {}, applyUpdates: () => {},
    updateScreenSize: () => {}, stopPhysics: () => {}, getState: () => ({ x: 400, y: 300 }) }
  const wsManager = { addClient: () => true, removeClient: () => null,
    getClients: () => [], getClientInfo: () => null }
  const logger = { info: () => {}, warn: () => {}, error: () => {},
    logSession: () => {}, debug: () => {} }
  const analytics = { recordSessionCreated: () => {}, recordSessionEnded: () => {},
    recordSessionError: () => {}, recordLanguage: () => {}, updatePeak: () => {},
    getStats: () => ({}) }
  return {
    repo, broadcast, physics, wsManager, logger, analytics,
    subscriptionService,
    createSession: async () => ({ id: 'test-session-auto' }),
    findOrCreateSession(id) { return repo.findOrCreateById(id) },
    getSession(id) { return repo.findById(id) },
    getSessionCount() { return repo.getAll().length },
    updateBallState() { return true },
    setLanguage() { return true },
    handleWebSocketConnection() {},
    handleWebSocketDisconnection() {},
    getClientInfo() { return null }
  }
}
...
async function main() {
  ...
  const logger = { info: (msg) => {}, warn: (msg) => {}, error: (msg) => {}, debug: () => {},
    logSession: () => {} }
  const analytics = { recordSessionCreated: () => {}, recordSessionEnded: () => {},
    recordSessionError: () => {}, recordLanguage: () => {}, updatePeak: () => {},
    getStats: () => ({}) }
  ...
  registerSubscriptionRoutes(app, subscriptionService, { logger, telegramBot: null })

  // Register session routes with subscription gating
  registerSessionRoutes(app, sessionService, apiCache, analytics, { requireSession: (r,s,n) => n(), logger }, subscriptionService)
  ...
  try {
    ...
    assert('GET /status returns active: false',
      res1a.data.active === false, JSON.stringify(res1a.data))

    const res1b = await httpPost(`/api/subscription/${CID_1}/check`)
    assert('POST /check returns active: false',
      res1b.data.active === false, JSON.stringify(res1b.data))

    // Reserve should fail with 402
    const res1c = await httpPost(`/api/session/${CID_1}/reserve`)
    assert('POST /reserve returns 402 (subscription required)',
      res1c.status === 402, `got ${res1c.status}: ${JSON.stringify(res1c.data)}`)
    ...
    assert('activate() returns success: true',
      actRes.success === true, JSON.stringify(actRes))

    const res2a = await httpGet(`/api/subscription/status/${TG_USER_1}`)
    assert('GET /status returns active: true',
      res2a.data.active === true, JSON.stringify(res2a.data))
    ...
    assert('POST /check(CID_1) returns active: true',
      res2b.data.active === true, JSON.stringify(res2b.data))
    assert('POST /check(CID_1) returns subscription.expiresAt',
      typeof res2b.data.subscription?.expiresAt === 'number', JSON.stringify(res2b.data))

    // Now reserve should succeed
    const res2c = await httpPost(`/api/session/${CID_1}/reserve`)
    assert('POST /reserve(CID_1) returns 200 (subscribed)',
      res2c.status === 200, `got ${res2c.status}: ${JSON.stringify(res2c.data)}`)
    assert('POST /reserve returns viewerUrl and controllerUrl',
      typeof res2c.data?.viewerUrl === 'string' && typeof res2c.data?.controllerUrl === 'string',
    ...
    assert('POST /check(CID_2) returns active: true (cross-computer)',
      res3a.data.active === true, JSON.stringify(res3a.data))

    const res3b = await httpPost(`/api/session/${CID_2}/reserve`)
    assert('POST /reserve(CID_2) returns 200 (cross-computer)',
      res3b.status === 200, `got ${res3b.status}: ${JSON.stringify(res3b.data)}`)

    // Verify CID_1 still works
    const res3c = await httpPost(`/api/subscription/${CID_1}/check`)
    assert('POST /check(CID_1) still active',
      res3c.data.active === true, JSON.stringify(res3c.data))
    ...
    assert('POST /activate-by-telegram returns 200',
      res5a.status === 200, JSON.stringify(res5a.data))
    assert('POST /activate-by-telegram success: true',
      res5a.data.success === true, JSON.stringify(res5a.data))

    // Now check the new customId
    const res5b = await httpPost(`/api/subscription/${CID_CROSS}/check`)
    assert('POST /check(cross) returns active: true',
      res5b.data.active === true, JSON.stringify(res5b.data))

    // Reserve with cross-computer customId
    const res5c = await httpPost(`/api/session/${CID_CROSS}/reserve`)
    assert('POST /reserve(cross) returns 200',
      res5c.status === 200, JSON.stringify(res5c.data))
    ...
    assert('POST /activate-by-telegram(unsubscribed) returns 402',
      res6a.status === 402, JSON.stringify(res6a.data))
    ...
    assert('GET /status has active: true',
      res7.data.active === true, JSON.stringify(res7.data))
    assert('GET /status has starsAmount: 75',
      res7.data.starsAmount === 75, JSON.stringify(res7.data))
    assert('GET /status customIds includes CID_1',
      Array.isArray(res7.data.customIds) && res7.data.customIds.includes(CID_1),
      JSON.stringify(res7.data.customIds))
    assert('GET /status customIds includes CID_2',
      res7.data.customIds.includes(CID_2),
      JSON.stringify(res7.data.customIds))
    assert('GET /status customIds includes CID_CROSS',
      res7.data.customIds.includes(CID_CROSS),
      JSON.stringify(res7.data.customIds))
    assert('GET /status has activatedAt timestamp',
      typeof res7.data.activatedAt === 'number', JSON.stringify(res7.data))
    ...
    assert('GET /status returns active: false after expiry',
      res8a.data.active === false, JSON.stringify(res8a.data))

    const res8b = await httpPost(`/api/subscription/${CID_1}/check`)
    assert('POST /check(CID_1) returns active: false after expiry',
      res8b.data.active === false, JSON.stringify(res8b.data))

    // Reserve should now fail
    const res8c = await httpPost(`/api/session/${CID_1}/reserve`)
    assert('POST /reserve returns 402 after expiry',
      res8c.status === 402, `got ${res8c.status}: ${JSON.stringify(res8c.data)}`)
