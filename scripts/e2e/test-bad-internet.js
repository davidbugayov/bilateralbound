...
const TEST_CONFIG = {
  ...
  networkProfiles: [
    ...
    { name: 'Very Poor', latency: 500, jitter: 200, packetLoss: 0.30 }
  ]
}
...
async function runStressTest() {
  ...
  try {
    ...
    for (const result of results) {
      console.log(`\n${result.profile}:`)
      console.log(`  Viewer Avg Jitter: ${result.viewerMetrics.avgJitter.toFixed(2)}px`)
      console.log(`  Viewer Max Jitter: ${result.viewerMetrics.maxJitter.toFixed(2)}px`)
