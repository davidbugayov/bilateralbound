...
function testBallAtLeftEdge() {
  ...
  const { px: wrongPx, py: wrongPy } = scalePreviewCoordinates(
    ballX, ballY, wrongViewerW, wrongViewerH, previewCanvasWidth, previewCanvasHeight
  )

  console.log(`  ❌ Wrong scaling: viewerW=${wrongViewerW}, viewerH=${wrongViewerH}`)
  console.log(`     Ball at (${ballX}, ${ballY}) -> Preview at (${wrongPx.toFixed(2)}, ${wrongPy.toFixed(2)})`)
  ...
  assert(wrongPercentX < expectedPercentX,
    'Ball should appear closer to center than it does with wrong scaling')

  console.log('  ✅ Bug confirmed: ball appears stuck at edge with wrong scaling')
}
...
function testCorrectScaling() {
  ...
  const { px: correctPx, py: correctPy } = scalePreviewCoordinates(
    ballX, ballY, correctViewerW, correctViewerH, previewCanvasWidth, previewCanvasHeight
  )

  console.log(`  ✅ Correct scaling: viewerW=${correctViewerW}, viewerH=${correctViewerH}`)
  console.log(`     Ball at (${ballX}, ${ballY}) -> Preview at (${correctPx.toFixed(2)}, ${correctPy.toFixed(2)})`)

  // Expected: ball should be at 2.5% from left edge (20/800 = 0.025)
  // With correct scaling: (20/800) * 400 = 10 pixels from left
  const expectedPx = (ballX / worldWidth) * previewCanvasWidth

  assert(Math.abs(correctPx - expectedPx) < 0.01,
...
function testPhysicsEngineBoundary() {
  ...
  assert(engine.ball.x >= 20,
    `Ball X should be >= 20, got ${engine.ball.x}`)

  // Direction should be reversed
  assert(engine.state.lastDirection.x > 0,
    'Direction X should be positive after hitting left wall')

  console.log(`  ✅ Ball at boundary: x=${engine.ball.x}, dirX=${engine.state.lastDirection.x}`)
}
...
function testPauseAtBoundary() {
  ...
  console.log(`  Server state: x=${serverState.x}, y=${serverState.y}, paused=${serverState.paused}`)
...
try {
  ...
  console.log('   - Impact: Ball appears stuck at edge when paused at boundary')
  console.log('   - Fix: Use actual worldWidth/worldHeight from viewerScreenSize')
