...
const PUBLIC_I18N_JS = path.join(ROOT, 'packages', 'web-client', 'public', 'js', 'i18n')

function generateI18n() {
  // Read src (canonical source for webpack)
  const lines = fs.readFileSync(path.join(SRC_I18N, 'i18n.js'), 'utf8').split('\n')
  const output = []
  ...
  for (const line of lines) {
    ...
    if (inExportBlock) {
      ...
      if (line.includes('typeof globalThis !== \'undefined\' && globalThis.document')) {
        output.push(line.replace('globalThis !== \'undefined\' && globalThis.document', 'root !== \'undefined\' && root.document'))
        inExportBlock = false
        continue
      }
...
function generateLanguageSelector() {
  const src = fs.readFileSync(path.join(SRC_I18N, 'language-selector.js'), 'utf8')
  ...
  fs.writeFileSync(path.join(PUBLIC_I18N_JS, 'language-selector.js'), content, 'utf8')
