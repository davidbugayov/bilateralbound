# BilateralBound - Система Версионирования

## Текущая Версия: v2.6.3
**Статус:** Активна
**Дата релиза:** 8 октября 2025 г. в 17:57
**Коммит:** 1da5b60

### Основные Изменения v2.6.3:
- ✨ fix: исправлена ошибка rate limiting с trustProxy

- Установлен trustProxy: true для корректной работы с nginx
- Добавлен fallback для получения IP адреса
- Исправлена ошибка ERR_ERL_UNEXPECTED_X_FORWARDED_FOR
- 🔄 Автоматическое обновление версии
- 🚀 Готов к развертыванию

### Предыдущие Версии:
- **v2.6.2** - Предыдущая версия

### Правила Версионирования:
1. **Major** (X.y.z) - Глобальные изменения архитектуры
2. **Minor** (x.Y.z) - Новые функции и оптимизации
3. **Patch** (x.y.Z) - Исправления ошибок

### Автоматическое Обновление:
- Версия обновляется при каждом значимом коммите
- Деплой происходит автоматически через GitHub Actions
- Мобильная версия скрывает индикатор версии

---
*Автоматически сгенерировано: 2025-10-08T14:57:17.896Z*
